import { A2AServer, } from "../../server/index.js"; // Import server components
import { ai } from "./genkit.js";
async function* coderAgent({ task, history, // Extract history from context
 }) {
    ...
    const fileContents = new Map(); // Store latest content per file
        ...
        for (const fileUpdate of codeChunk.files) {
            ...
            if (!fileOrder.includes(fileUpdate.filename)) {
                ...
                if (currentFileOrderIndex > 0 &&
                    ...
                    console.log(`[CoderAgent] Emitting completed file (index ${prevFileIndex}): ${prevFilename}`);
                    yield {
                        index: prevFileIndex,
                        name: prevFilename,
    ...
    const fullMessage = (await response).output; // Add undefined check
