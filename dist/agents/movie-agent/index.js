 ...
 */
async function* movieAgentHandler(context) {
    console.log(`[MovieAgent] Processing task ${context.task.id} with state ${context.task.status.state}`);
    ...
    const messages = (context.history ?? []) // Use history if available, default to empty array
        .map((m) => ({
        // Map roles explicitly and assert the type for Genkit
        role: (m.role === "agent" ? "model" : "user"),
        content: m.parts
            .filter((p) => !!p.text) // Filter for text parts
            .map((p) => ({
            text: p.text,
        })),
    }))
    ...
    if (messages.length === 0) {
        console.warn(`[MovieAgent] No valid text messages found in history for task ${context.task.id}. Cannot proceed.`);
        yield {
            state: "failed",
            message: {
                role: "agent",
    ...
    const goal = context.task.metadata?.goal;
    try {
        // Run the Genkit prompt
        const response = await movieAgentPrompt({ goal: goal, now: new Date().toISOString() }, // Pass goal from metadata
        {
            messages,
        ...
        if (finalStateLine === "COMPLETED") {
            ...
            console.warn(`[MovieAgent] Unexpected final state line from prompt: ${finalStateLine}. Defaulting to 'completed'.`);
        ...
        console.log(`[MovieAgent] Task ${context.task.id} finished with state: ${finalState}`);
    }
    catch (error) {
        console.error(`[MovieAgent] Error processing task ${context.task.id}:`, error);
