...
export function rollDice(sides = 20, rolls = 1, advantage = 'normal') {
    const results = [];
...
export function calculateDC(difficulty) {
    switch (difficulty) {
        case 'easy': return 10;
 ...
 */
export function performSkillCheck(characterName, skill, difficulty = 'medium', advantage = 'normal') {
 ...
 */
export function resolveAction(characterName, action, targetName, targetObject, tavernState) {
...
function hasBobDetectedTheft(tavernState) {
    // Check recent events for Bob detecting theft
    const recentEvents = tavernState.events.slice(-5);
    return recentEvents.some(event => event.description.toLowerCase().includes('bob') &&
        event.description.toLowerCase().includes('suspicious'));
}
/**
 * Combat encounter between characters
 */
export function resolveCombatAction(attacker, defender, actionType) {
