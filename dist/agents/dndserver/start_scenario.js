 ...
 */
async function sendTavernRequest(message) {
    try {
        ...
        if (responseData && typeof responseData === 'object' && 'error' in responseData) {
            throw new Error(`Tavern server returned an error: ${JSON.stringify(responseData.error)}`);
        }
 ...
 */
async function initializeScenario(scenarioName, time = "Evening", atmosphere = "Warm and inviting", additionalObjects = []) {
    try {
 ...
 */
async function setCharacterGoals(goals) {
    try {
 ...
 */
async function startInteraction(maxTurns = 5) {
    try {
 ...
 */
async function getTavernStateAndLog() {
    try {
 ...
 */
async function runScenario(scenarioName, goals, maxTurns = 5, time = "Evening", atmosphere = "Warm and inviting", additionalObjects = []) {
    try {
...
export { initializeScenario, setCharacterGoals, startInteraction, getTavernStateAndLog, displayConversations, runScenario };
