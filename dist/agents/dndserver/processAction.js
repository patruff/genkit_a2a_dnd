 ...
 */
export function createSkillCheckResult(characterName, skillCheck) {
    return {
        characterName,
 ...
 */
export async function processAction(request, activeCharacter, activeAgent, modifiedResponse, currentTavernState, tavernLog, characterGoal) {
    try {
        // Extract the action tag and everything before it
        const actionTag = modifiedResponse.match(new RegExp(`\\[\\s*ACTION\\s*:\\s*${request.actionType}\\s*.*?\\]`, 'i'))?.[0];
        if (!actionTag) {
        ...
        if (request.target) {
            ...
            const targetCharMatch = currentTavernState.characters.find(c => c.name.toLowerCase() === request.target?.toLowerCase());
            if (targetCharMatch) {
        ...
        const actionResult = gameEngine.resolveAction(activeCharacter, request.actionType, targetCharacter, targetObject, currentTavernState);
