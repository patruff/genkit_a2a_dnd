...
let currentTavernState = { ...DEFAULT_TAVERN_STATE };
...
function logCharacterAction(character, action) {
...
function logConversation(speaker, listener, message) {
    const timestamp = new Date().toISOString();
...
async function runInteractionCycle(userMessage) {
 ...
 */
async function* tavernServerHandler(context) {
    console.log(`[TavernServer] Processing task ${context.task.id} with state ${context.task.status.state}`);
    ...
    if (messages.length === 0) {
        console.warn(`[TavernServer] No valid text messages found in history for task ${context.task.id}.`);
        yield {
            state: "failed",
            message: {
                role: "agent",
    ...
    try {
        ...
        if (setGoalMatch) {
            ...
            const character = currentTavernState.characters.find(c => c.name.toLowerCase() === characterName.toLowerCase());
            if (character) {
        ...
        if (userRequest.toLowerCase().includes("run interaction") ||
        ...
        if (finalStateLine === "COMPLETED") {
            ...
            console.warn(`[TavernServer] Unexpected final state line from prompt: ${finalStateLine}. Defaulting to 'input-required'.`);
            finalState = "input-required";
        }
        ...
        console.log(`[TavernServer] Task ${context.task.id} finished with state: ${finalState}`);
    }
    catch (error) {
        console.error(`[TavernServer] Error processing task ${context.task.id}:`, error);
        yield {
            state: "failed",
            message: {
                role: "agent",
...
// --- Server Setup ---
const tavernServerCard = {
    name: "The Tipsy Gnome Tavern",
    description: "A Dungeons & Dragons tavern server that manages interactions between multiple character agents, including Homie the gnome thief and Bob the bartender.",
    ...
    skills: [
        {
            ...
            description: "Manages a virtual D&D tavern environment with multiple character agents who can interact with each other and the user.",
