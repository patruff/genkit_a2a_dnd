...
function printAgentEvent(event) {
    ...
    // Check if it's a TaskArtifactUpdateEvent
        ...
        const update = event; // Cast for type safety
        console.log(`${prefix} 📄 Artifact Received: ${update.artifact.name || "(unnamed)"} (Index: ${update.artifact.index ?? 0})`);
...
function printMessageContent(message) {
    message.parts.forEach((part, index) => {
        ...
        if ("text" in part) {
            ...
            const filePart = part;
            console.log(`${partPrefix} ${colorize("blue", "📄 File:")} Name: ${filePart.file.name || "N/A"}, Type: ${filePart.file.mimeType || "N/A"}, Source: ${filePart.file.bytes ? "Inline (bytes)" : filePart.file.uri}`);
...
async function fetchAndDisplayAgentCard() {
    ...
    console.log(colorize("dim", `Attempting to fetch agent card from: ${wellKnownUrl}`));
    try {
        ...
        if (response.ok) {
            ...
            console.log(colorize("yellow", `⚠️ Could not fetch agent card (Status: ${response.status})`));
        }
    }
    catch (error) {
        console.log(colorize("yellow", `⚠️ Error fetching agent card: ${error.message}`));
    }
}
// --- Main Loop ---
async function main() {
    ...
    rl.on("line", async (line) => {
        ...
        try {
            ...
            console.error(colorize("red", `\n❌ Error communicating with agent (${agentName}):`), error.message || error);
            if (error.code) {
