...
import fs from 'fs';
import { MCPRegistry, readFutureFile, writeFutureFile } from './src/mcp-client.js';
...
async function runEnhancedScenario() {
  ...
  // Step 0: Initialize MCP or fallback
  printStepHeader(0, "Initializing MCP or Fallback");
  
  // Define file paths
  const futurePath = 'future.txt';
  const localFuturePath = path.join(process.env.MCP_ALLOWED_DIR || '/mnt/c/Users/patru/anthropicFun', 'future.txt'); 
  
  // Try to set up MCP but prepare for fallback
...
try {
  ...
  if (fs.existsSync(configPath)) {
    console.log(formatMessage('mcp', `Loading MCP config from ${configPath}`));
    mcpRegistry = await loadMCPRegistry(configPath);
    console.log(formatMessage('mcp', `Loaded MCP configurations: ${mcpRegistry.listMCPs().join(', ')}`));
    // Try to start the filesystem MCP
    console.log(formatMessage('mcp', 'Starting filesystem MCP...'));
    filesystemMCP = await mcpRegistry.startMCP('filesystem');
    console.log(formatMessage('mcp', 'Filesystem MCP started successfully'));
    
    // Test if MCP is working by writing a test file to the local directory
    const testFilePath = path.join(process.cwd(), 'mcp_test.txt');
    console.log(formatMessage('mcp', `Testing MCP by writing to ${testFilePath}`));
    await filesystemMCP.callTool('write_file', {
      path: testFilePath,
      content: 'MCP filesystem test - ' + new Date().toISOString()
    });
    console.log(formatMessage('mcp', 'Successfully wrote test file using MCP'));
  } else {
    console.log(formatMessage('mcp', `MCP config not found, using direct file access fallback`));
    useMcpFallback = true;
  }
} catch (error) {
  console.error(formatMessage('mcp', `Error initializing MCP: ${error.message}`));
  console.log(formatMessage('mcp', 'Falling back to direct file access'));
  useMcpFallback = true;
}
  ...
  if (useMcpFallback) {
    console.log(formatMessage('mcp', `Operating in fallback mode with direct file access`));
    console.log(formatMessage('mcp', `Using future file at: ${localFuturePath}`));
  } else {
    console.log(formatMessage('mcp', `Operating with MCP server`));
    console.log(formatMessage('mcp', `Using future file: ${futurePath} via MCP`));
  }
  
  // Initialize the future file
  const initialFuture = "Homie will try to steal Bob's gem tonight!";
  await writeFutureWithFallback(filesystemMCP, futurePath, localFuturePath, initialFuture);
  console.log(formatMessage('mcp', 'Future file initialized successfully'));
  ...
  // Step 6: WZA accesses the future via MCP or fallback
  printStepHeader(6, "WZA uses future sight powers");
  
  // Ensure future.txt exists with our prediction
  const barFightPrediction = "Homie will steal Bob's gem tonight while Bob is distracted by a bar fight!";
  await writeFutureWithFallback(
    filesystemMCP, 
    futurePath,
    localFuturePath,
    barFightPrediction
  ...
  const futureContent = await readFutureWithFallback(filesystemMCP, futurePath, localFuturePath);
  ...
  const friendshipFuture = "Homie and Bob become friends, and Homie helps protect the gem from other thieves";
  await writeFutureWithFallback(
    filesystemMCP,
    futurePath,
    localFuturePath,
  ...
  const finalFuture = await readFutureWithFallback(filesystemMCP, futurePath, localFuturePath);
  
  console.log(chalk.bold.green(`"${finalFuture}"`));
  console.log(chalk.bold('='.repeat(80)) + '\n');
  
  // Cleanup MCP processes if they were started
  if (mcpRegistry) {
    console.log(formatMessage('mcp', 'Stopping MCP processes...'));
...
// Direct file access functions as fallback
async function readFutureFileDirectly(filePath) {
  try {
    console.log(formatMessage('mcp', `Directly reading future from ${filePath}`));
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, 'utf8');
      console.log(formatMessage('mcp', `Future content: ${content}`));
      return content;
    } else {
      console.log(formatMessage('mcp', `Future file not found, creating default`));
      const defaultContent = "Homie will try to steal Bob's gem tonight!";
      fs.writeFileSync(filePath, defaultContent, 'utf8');
      return defaultContent;
    }
  } catch (error) {
    console.error(formatMessage('mcp', `Error reading future: ${error.message}`));
    return "The future is unclear";
  }
}

async function writeFutureFileDirectly(filePath, content) {
  try {
    console.log(formatMessage('mcp', `Directly changing future to: ${content}`));
    fs.writeFileSync(filePath, content, 'utf8');
    console.log(formatMessage('mcp', `Future successfully changed`));
    return true;
  } catch (error) {
    console.error(formatMessage('mcp', `Error changing future: ${error.message}`));
    return false;
  }
}

// Helper function to load MCP Registry with error handling and fallback
async function loadMCPRegistry(configPath) {
  try {
    if (!fs.existsSync(configPath)) {
      console.error(`MCP config file not found: ${configPath}`);
    ...
    console.error(`Error loading MCP registry: ${error.message}`);
    throw error;
  }
}

// Create helper functions with fallback
async function readFutureWithFallback(mcpProcess, futurePath, localPath) {
  if (mcpProcess) {
    try {
      return await readFutureFile(mcpProcess, futurePath);
    } catch (error) {
      console.error(`Error reading future with MCP, using direct fallback: ${error.message}`);
      return await readFutureFileDirectly(localPath);
    }
  } else {
    return await readFutureFileDirectly(localPath);
  }
...
async function writeFutureWithFallback(mcpProcess, futurePath, localPath, content) {
  if (mcpProcess) {
    try {
      return await writeFutureFile(mcpProcess, futurePath, content);
    } catch (error) {
      console.error(`Error writing future with MCP, using direct fallback: ${error.message}`);
      return await writeFutureFileDirectly(localPath, content);
    }
  } else {
    return await writeFutureFileDirectly(localPath, content);
  }
...
// Run the scenario
try {
  console.log(chalk.bold('Starting the enhanced WZA scenario with MCP integration...\n'));
